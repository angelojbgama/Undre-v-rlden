...
class TileSemanticEditor(QWidget):
    ...
    def set_selection(self, tileset_id: str, source_index: int) -> None:
        ...
        if semantic:
            self.semantic_id.setText(semantic.definition_id); self.family.setText(semantic.family); self._set(self.role, semantic.role); self._set(self.topology, semantic.topology); self.preferred_layer.setText(semantic.preferred_layer); self.variant_weight.setValue(max(1, semantic.variant_weight)); self._set(self.north, semantic.north); self._set(self.east, semantic.east); self._set(self.south, semantic.south); self._set(self.west, semantic.west); self.flip_x.setChecked(semantic.flip_x_allowed)
        else:
            self.semantic_id.setText(f"semantic.{tileset_id}.{self.source_index}"); self.family.clear(); self._set(self.role, "unknown"); self._set(self.topology, "unknown"); self.preferred_layer.clear(); self.variant_weight.setValue(1); [self._set(widget, "unknown") for widget in (self.north, self.east, self.south, self.west)]; self.flip_x.setChecked(False)
