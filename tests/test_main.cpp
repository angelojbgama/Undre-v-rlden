...
void testSemanticAuthoringFoundation() {
    ...
    const auto* frameNorthWest = semantics.findTile(
        simulation::DefinitionId{"tileset.dungeon"}, 2U + 2U * 19U);
    ...
    expect(semantics.edgesCompatible(game::authoring::EdgeProfile::masonry,
                                     game::authoring::EdgeProfile::masonry) &&
               !semantics.edgesCompatible(game::authoring::EdgeProfile::masonry,
                                          game::authoring::EdgeProfile::floor) &&
               semantics.edgesCompatible(game::authoring::EdgeProfile::unknown,
                                          game::authoring::EdgeProfile::floor),
           "edge compatibility distinguishes known mismatch from unknown evidence");

    const auto* stamp = semantics.findStamp(
        simulation::DefinitionId{"stamp.dungeon.masonry_frame_3x3"});
    maps::MapData semanticMap;
    semanticMap.id = simulation::MapId{"map.semantic"};
    semanticMap.width = 8;
    semanticMap.height = 8;
    semanticMap.tileSize = 16;
    semanticMap.layers.push_back(
        {"ground", true, std::vector<std::optional<std::uint32_t>>(64)});
    semanticMap.collision.assign(64, 0);
    if (stamp) {
        for (const auto& cell : stamp->cells) {
            const auto* definition = semantics.findTile(cell.tileId);
            if (!definition) { continue; }
            semanticMap.tileReferences.push_back(
                game::authoring::tileReferenceFor(*definition));
            const auto index = static_cast<std::size_t>(2 + cell.x) +
                               static_cast<std::size_t>(2 + cell.y) * semanticMap.width;
            semanticMap.layers[0].cells[index] =
                static_cast<std::uint32_t>(semanticMap.tileReferences.size() - 1U);
        }
    }
    ...
    expect(stamp && std::any_of(broken.issues.begin(), broken.issues.end(), [](const auto& issue) {
        return issue.code == "broken_atomic_stamp";
    }), "semantic validator reports a confidently incomplete atomic stamp as a warning");

    maps::MapData sparseMap;
    sparseMap.id = simulation::MapId{"map.semantic.sparse"};
    sparseMap.width = 8;
    sparseMap.height = 8;
    sparseMap.tileSize = 16;
    sparseMap.layers.push_back(
        {"ground", true, std::vector<std::optional<std::uint32_t>>(64)});
    sparseMap.collision.assign(64, 0);
    const auto* frameNorth = semantics.findTile(
        simulation::DefinitionId{"tileset.dungeon"}, 3U + 2U * 19U);
    if (frameNorthWest && frameNorth) {
        sparseMap.tileReferences.push_back(
            game::authoring::tileReferenceFor(*frameNorthWest));
        sparseMap.tileReferences.push_back(game::authoring::tileReferenceFor(*frameNorth));
        sparseMap.layers[0].cells[0] = 0U;
        sparseMap.layers[0].cells[1] = 1U;
    }
    const auto sparseReport = validator.validate(sparseMap, semantics);
    expect(frameNorth && !std::any_of(
               sparseReport.issues.begin(), sparseReport.issues.end(), [](const auto& issue) {
                   return issue.code == "broken_atomic_stamp";
               }),
           "sparse reusable stamp members do not produce a broken-stamp warning");

    semanticMap.tileReferences.push_back(
        {simulation::DefinitionId{"tileset.dungeon"}, 0, world::TileFlags::none});
    semanticMap.layers[0].cells[0] =
        static_cast<std::uint32_t>(semanticMap.tileReferences.size() - 1U);
    ...
    expect(std::any_of(unclassified.issues.begin(), unclassified.issues.end(), [](const auto& issue) {
        return issue.code == "unclassified_tile" &&
               issue.severity == game::authoring::SemanticIssueSeverity::info;
...
void testUiScreenBuiltinAndBindings() {
    ...
    expect(builtin.uiScreens.size() == 12 &&
               ...
               builtin.uiScreens.front().root.children[2].id == "hud.health" &&
               builtin.uiScreens.front().root.children[2].meter &&
               builtin.uiScreens.front().root.children[2].meter->sprites.full &&
               builtin.uiScreens.front().root.children[2].meter->spacing == 1,
           "builtin content authors the complete HUD as an authored screen");
    // The compact HUD has no full-width bars; the gear strip marks what each
    // button activates (sword Z, shield armor, bow X).
    const auto& hudNodes = builtin.uiScreens.front().root.children;
    const auto hasNode = [&hudNodes](const char* id) {
        return std::any_of(hudNodes.begin(), hudNodes.end(),
                           [&](const auto& node) { return node.id == id; });
    };
    expect(!hasNode("hud.topBar") && !hasNode("hud.bottomBar") && !hasNode("hud.hint") &&
               hasNode("hud.gear.sword") && hasNode("hud.gear.swordKey") &&
               hasNode("hud.gear.shield") && hasNode("hud.gear.bow") &&
               hasNode("hud.gear.bowKey"),
           "builtin HUD drops the full-width bars and authors the gear strip");

    const auto compiled = content::compileContent(builtin);
    expect(compiled && compiled.registry->uiScreens().values().size() == 12 &&
               compiled.registry->uiScreens().find({"screen.hud"}) != nullptr &&
               compiled.registry->uiScreens().require({"screen.hud"}).root.children[2]
                   .meter->spacing == 1,
