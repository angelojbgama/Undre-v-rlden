...
enum MenuId : UINT { fileNew=1001,fileOpen,fileSave,fileSaveAs,fileSaveAll,fileExit,editUndo,editRedo,viewGrid,viewFrame,viewMap,viewContent,viewValidate };
...
public:
    ...
    int run(){
        ...
        window_=CreateWindowExW(0,className,L"Dungeon Underworld - Content Studio",WS_OVERLAPPEDWINDOW,CW_USEDEFAULT,CW_USEDEFAULT,1200,800,nullptr,createMenu(),instance_,this);
        if(!window_)return 1;ShowWindow(window_,show_);UpdateWindow(window_);SetTimer(window_,autosaveTimerId,5000,nullptr);SetTimer(window_,previewTimerId,16,nullptr);
    ...
    ~EditorWindow(){if(window_&&IsWindow(window_)){KillTimer(window_,autosaveTimerId);KillTimer(window_,previewTimerId);DestroyWindow(window_);}if(registered_)UnregisterClassW(className,instance_);}
private:
    HMENU createMenu(){HMENU bar=CreateMenu(),file=CreatePopupMenu(),edit=CreatePopupMenu(),view=CreatePopupMenu();AppendMenuW(file,MF_STRING,fileNew,L"&New\tCtrl+N");AppendMenuW(file,MF_STRING,fileOpen,L"&Open Map...");AppendMenuW(file,MF_STRING,fileSave,L"&Save\tCtrl+S");AppendMenuW(file,MF_STRING,fileSaveAs,L"Save &As...");AppendMenuW(file,MF_STRING,fileSaveAll,L"Save &All");AppendMenuW(file,MF_SEPARATOR,0,nullptr);AppendMenuW(file,MF_STRING,fileExit,L"E&xit");AppendMenuW(edit,MF_STRING,editUndo,L"&Undo\tCtrl+Z");AppendMenuW(edit,MF_STRING,editRedo,L"&Redo\tCtrl+Y");AppendMenuW(view,MF_STRING,viewMap,L"&Map Mode");AppendMenuW(view,MF_STRING,viewContent,L"&Content Mode");AppendMenuW(view,MF_SEPARATOR,0,nullptr);AppendMenuW(view,MF_STRING,viewGrid,L"&Grid");AppendMenuW(view,MF_STRING,viewFrame,L"&Frame Map\tHome");AppendMenuW(view,MF_STRING,viewValidate,L"&Validate Workspace");AppendMenuW(bar,MF_POPUP,reinterpret_cast<UINT_PTR>(file),L"&File");AppendMenuW(bar,MF_POPUP,reinterpret_cast<UINT_PTR>(edit),L"&Edit");AppendMenuW(bar,MF_POPUP,reinterpret_cast<UINT_PTR>(view),L"&View");return bar;}
    ...
    void menuCommand(UINT id){if(id==fileNew){if(confirmUnsaved())app_.shellCommand(EditorShellCommand::newMap);}else if(id==fileOpen){if(confirmUnsaved())openFile();}else if(id==fileSave)save(false);else if(id==fileSaveAs)save(true);else if(id==fileSaveAll){std::string error;if(!app_.saveAll(error))showError(error);}else if(id==fileExit)SendMessageW(window_,WM_CLOSE,0,0);else if(id==editUndo)app_.shellCommand(EditorShellCommand::undo);else if(id==editRedo)app_.shellCommand(EditorShellCommand::redo);else if(id==viewMap)app_.shellCommand(EditorShellCommand::mapMode);else if(id==viewContent)app_.shellCommand(EditorShellCommand::contentMode);else if(id==viewGrid)app_.shellCommand(EditorShellCommand::toggleGrid);else if(id==viewFrame)app_.shellCommand(EditorShellCommand::frameMap);else if(id==viewValidate)app_.shellCommand(EditorShellCommand::validateWorkspace);}
    std::optional<std::filesystem::path> fileDialog(bool saveDialog){wchar_t buffer[32768]{};OPENFILENAMEW dialog{};dialog.lStructSize=sizeof(dialog);dialog.hwndOwner=window_;dialog.lpstrFilter=L"Dungeon authored/runtime maps (*.umap;*.dmap)\0*.umap;*.dmap\0All files\0*.*\0";dialog.lpstrFile=buffer;dialog.nMaxFile=static_cast<DWORD>(std::size(buffer));dialog.lpstrDefExt=L"umap";dialog.Flags=OFN_EXPLORER|OFN_PATHMUSTEXIST|(saveDialog?OFN_OVERWRITEPROMPT:OFN_FILEMUSTEXIST);const BOOL result=saveDialog?GetSaveFileNameW(&dialog):GetOpenFileNameW(&dialog);return result?std::optional<std::filesystem::path>{buffer}:std::nullopt;}
    ...
    bool confirmUnsaved(){if(!app_.hasUnsavedChanges())return true;const int choice=MessageBoxW(window_,L"Save changes before continuing?",L"Dungeon Underworld - Content Studio",MB_YESNOCANCEL|MB_ICONWARNING);if(choice==IDCANCEL)return false;if(choice==IDYES)return save(false);return true;}
    void showError(const std::string& error){const std::wstring wide(error.begin(),error.end());MessageBoxW(window_,wide.c_str(),L"Content Studio",MB_OK|MB_ICONERROR);}
