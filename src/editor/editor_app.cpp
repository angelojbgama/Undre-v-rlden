...
void EditorApp::drawShell(EditorUiContext& ui,const EditorInputState& input){
    ...
    if(ui.button({8,y,85,18},"COLL RECT",document_.activeTool()==EditorTool::collisionRectangle))document_.activeTool()=EditorTool::collisionRectangle;
    if(ui.button({97,y,85,18},"COLL FILL",document_.activeTool()==EditorTool::collisionFill))document_.activeTool()=EditorTool::collisionFill;y+=24;
...
void EditorApp::handleViewport(core::RectI viewport,const EditorInputState& input){
    ...
    if(input.pointer.leftPressed&&inside){
        ...
        else if((tool==EditorTool::tileRectangle||tool==EditorTool::collisionRectangle) && tile){drag_.kind=DragState::Kind::rectangle;drag_.worldStart={static_cast<int>(tile->x),static_cast<int>(tile->y)};drag_.worldCurrent=drag_.worldStart;}
        ...
        else if(tool==EditorTool::collisionFill && tile)execute(std::make_unique<SetCollisionCommand>(collisionFloodCells(document_.data(),tile->x,tile->y),true));
    ...
    if(input.pointer.leftReleased){
        ...
        else if(drag_.kind==DragState::Kind::rectangle){const auto cells=rectangleCells(drag_.worldStart.x,drag_.worldStart.y,drag_.worldCurrent.x,drag_.worldCurrent.y,document_.data());if(tool==EditorTool::collisionRectangle)execute(std::make_unique<SetCollisionCommand>(cells,true));else execute(std::make_unique<PaintTilesCommand>(document_.activeLayer(),cells,maps::MapTileReference{simulation::DefinitionId{"tileset.dungeon"},selectedTile_,tileFlipX_?world::TileFlags::flipX:world::TileFlags::none}));}
