...
void EditorApp::drawShell(EditorUiContext& ui,const EditorInputState& input){
    ...
    for(std::size_t i=0;i<document_.data().layers.size();++i){
        ...
        if(ui.button({8,y,174,18},layer.name,i==document_.activeLayer()))document_.activeLayer()=i;
        y+=20;
    }
    ...
    if(tileset_){const int paletteTop=std::max(y+24,viewportHeight-204);ui.label("TILES",8,paletteTop-12);
        for(std::uint32_t index=0;index<228;++index){const int column=static_cast<int>(index%10),row=static_cast<int>(index/10);const core::RectI cell{8+column*17,paletteTop+row*17,16,16};if(cell.y+16>viewportHeight)break;
...
void EditorApp::drawEntities(render::Renderer2D& renderer,core::RectI viewport) const{
    ...
    for(const auto& value:document_.data().playerSpawns){const auto p=worldToScreen(value.position,viewport);renderer.fillRect({p.x-5,p.y-5,11,11},categoryColor(SelectionKind::playerSpawn));}
    for(const auto& value:document_.data().links){const auto p=worldToScreen({value.trigger.x,value.trigger.y},viewport);const int w=std::max(2,static_cast<int>(value.trigger.width*zoom())),h=std::max(2,static_cast<int>(value.trigger.height*zoom()));outline(renderer,{p.x,p.y,w,h},categoryColor(SelectionKind::mapLink));}
...
void EditorApp::handleViewport(core::RectI viewport,const EditorInputState& input){
    ...
    const auto tile=TileCoordinate{static_cast<std::uint32_t>(std::max(0,worldPoint.x)/tileSize),static_cast<std::uint32_t>(std::max(0,worldPoint.y)/tileSize)};
    const auto tool=document_.activeTool();
    if(input.pointer.leftPressed&&inside){
        if(tool==EditorTool::tilePencil||tool==EditorTool::tileErase||tool==EditorTool::collisionPaint||tool==EditorTool::collisionErase){drag_.kind=DragState::Kind::brush;drag_.stroke={tile};}
        else if(tool==EditorTool::tileRectangle){drag_.kind=DragState::Kind::rectangle;drag_.worldStart={static_cast<int>(tile.x),static_cast<int>(tile.y)};drag_.worldCurrent=drag_.worldStart;}
        else if(tool==EditorTool::tileFill){if(input.alt){const auto index=static_cast<std::size_t>(tile.y)*document_.data().width+tile.x;if(tile.x<document_.data().width&&tile.y<document_.data().height){const auto cell=document_.data().layers[document_.activeLayer()].cells[index];if(cell){const auto& ref=document_.data().tileReferences[*cell];selectedTile_=ref.sourceIndex;tileFlipX_=world::hasFlag(ref.flags,world::TileFlags::flipX);}}}else execute(std::make_unique<PaintTilesCommand>(document_.activeLayer(),tileFloodCells(document_.data(),document_.activeLayer(),tile.x,tile.y),maps::MapTileReference{simulation::DefinitionId{"tileset.dungeon"},selectedTile_,tileFlipX_?world::TileFlags::flipX:world::TileFlags::none}));}
    ...
    if(input.pointer.leftReleased){
        ...
        else if(drag_.kind==DragState::Kind::rectangle)execute(std::make_unique<PaintTilesCommand>(document_.activeLayer(),rectangleCells(drag_.worldStart.x,drag_.worldStart.y,drag_.worldCurrent.x,drag_.worldCurrent.y,document_.data()),maps::MapTileReference{simulation::DefinitionId{"tileset.dungeon"},selectedTile_,tileFlipX_?world::TileFlags::flipX:world::TileFlags::none}));
        else if(drag_.kind==DragState::Kind::move&&drag_.worldCurrent!=drag_.entityStart)execute(std::make_unique<MoveEntityCommand>(document_.selection().kind,document_.selection().instanceId,drag_.entityStart,drag_.worldCurrent));
    ...
    if(input.duplicatePressed&&document_.selection().instanceId){const auto id=document_.allocatePersistentId();const auto copy=duplicatePlacement(document_,document_.selection().kind,document_.selection().instanceId,id,tileSize);if(copy)execute(std::make_unique<PlaceEntityCommand>(*copy));}
...
void EditorApp::drawInspector(EditorUiContext& ui,core::RectI panel){int y=10;ui.label("PROPERTIES",panel.x+8,y);y+=18;ui.label(std::string(document_.data().id.value()),panel.x+8,y);y+=18;
    ...
    y=panel.height-150;ui.label("VALIDATION",panel.x+8,y);y+=16;const auto report=document_.validate(content_);ui.label(std::to_string(report.errorCount())+" errors",panel.x+8,y);y+=14;ui.label(std::to_string(report.warningCount())+" warnings",panel.x+8,y);y+=16;for(const auto& issue:report.issues){if(y>panel.height-12)break;ui.label(issue.message.substr(0,32),panel.x+8,y);y+=12;}
}
