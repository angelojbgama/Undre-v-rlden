...
void Renderer2D::drawImageRegionTinted(const Image& image, core::RectI source, int destinationX,
                                       int destinationY, core::ColorRGBA8 tint, bool flipX) {
        ...
        for (int x = clippedLeft; x < clippedRight; ++x) {
            ...
            const core::ColorRGBA8 sourceColor{
                static_cast<std::uint8_t>(
                    (bytes[sourceOffset] * tint.r + 127U) / 255U),
                static_cast<std::uint8_t>(
                    (bytes[sourceOffset + 1U] * tint.g + 127U) / 255U),
                static_cast<std::uint8_t>(
                    (bytes[sourceOffset + 2U] * tint.b + 127U) / 255U),
