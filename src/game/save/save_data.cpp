...
std::string validateSaveData(const SaveData& data,const SaveValidationCatalogs& catalogs){
    ...
    if(data.player.health<0||data.player.health>progression->baseStats.maximumHealth)return "saved player health is invalid";
...
std::vector<std::uint8_t> serializeSave(const SaveData& data){
    ...
    const auto quests=data.quests.snapshot();if(!quests.empty()){ByteWriter state;state.writeU32(static_cast<std::uint32_t>(quests.size()));for(const auto& quest:quests){state.writeU32(stringIndex(strings,quest.questId.value()));state.writeU8(static_cast<std::uint8_t>(quest.status));state.writeU32(static_cast<std::uint32_t>(quest.objectives.size()));for(const auto& objective:quest.objectives){state.writeU32(stringIndex(strings,objective.objectiveId.value()));state.writeU32(objective.currentCount);}}chunk(chunks,"QSTS",std::move(state));}
