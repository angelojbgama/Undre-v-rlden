...
std::string validateSaveData(const SaveData& data,const SaveValidationCatalogs& catalogs){
    ...
    for(const auto& delta:data.world.objects){if(delta.key.mapId.empty()||!delta.key.instanceId||!keys.emplace(delta.key).second)return "duplicate or invalid object delta key";if(delta.doorState&&!validDoorState(*delta.doorState))return "object delta has an invalid door state";const auto* map=findMap(catalogs,delta.key.mapId);if(!map||!hasInstance(*map,delta.key.instanceId,true))return "object delta references unknown map instance";if(delta.doorState&&catalogs.objects){const auto placement=std::find_if(map->objects.begin(),map->objects.end(),[&](const auto& value){return value.id==delta.key.instanceId;});const auto* definition=placement==map->objects.end()?nullptr:catalogs.objects->find(placement->definitionId);if(!definition||!definition->door)return "door delta references a non-door object";}for(const auto& stack:delta.remainingContents)if(!validStack(stack,*catalogs.items))return "object delta stack is invalid";}
...
std::vector<std::uint8_t> serializeSave(const SaveData& data){
    ...
    deltas.writeU32(static_cast<std::uint32_t>(objects.size()));for(const auto& value:objects){deltas.writeU32(stringIndex(strings,value.key.mapId.value()));deltas.writeU64(value.key.instanceId.value);deltas.writeU8(value.opened?1:0);deltas.writeU8(value.destroyed?1:0);deltas.writeU8(value.doorState?1:0);if(value.doorState)deltas.writeU8(static_cast<std::uint8_t>(*value.doorState));deltas.writeU32(static_cast<std::uint32_t>(value.remainingContents.size()));for(const auto& stack:value.remainingContents){deltas.writeU32(stringIndex(strings,stack.itemId.value()));deltas.writeU32(stack.quantity);}}
...
bool applyWorldState(const SessionWorldState& state,maps::RuntimeWorld& world,simulation::EntityHandlePool& handles,const gameplay::ItemCatalog& items,std::string& error){
    auto& objects=world.objects();for(auto it=objects.begin();it!=objects.end();){const auto* delta=state.findObject({world.id(),it->persistentId});if(!delta){++it;continue;}if(delta->destroyed){static_cast<void>(handles.destroy(it->instance.handle()));it=objects.erase(it);continue;}if(delta->doorState)static_cast<void>(world.setDoorState(it->persistentId,*delta->doorState));if(delta->opened)static_cast<void>(it->instance.open());if(auto* contents=it->instance.contents()){for(std::size_t index=0;index<contents->capacity();++index){if(const auto slot=contents->slot(index))static_cast<void>(contents->remove(slot->itemId,slot->quantity));}for(const auto& stack:delta->remainingContents){const auto added=contents->add(stack.itemId,stack.quantity);if(added.remainder){error="saved object contents do not fit";return false;}}}++it;}
...
void captureWorldState(const maps::MapData& original, const maps::RuntimeWorld& world,
    ...
    for (const auto& placement : original.objects) {
        ...
        if (runtime == world.objects().end()) {
            state.set(ObjectDelta{{original.id, placement.id}, false, true, {}, std::nullopt});
            continue;
        }
