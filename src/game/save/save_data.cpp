...
std::vector<std::string> stringsFor(const SaveData& data){
    ...
    for (const auto& crafted : data.craftedRecipes) add(crafted.recipeId.value());
...
std::string validateSaveData(const SaveData& data,const SaveValidationCatalogs& catalogs){
    ...
    if (!data.craftedRecipes.empty() && !catalogs.crafting)
        return "save validation requires crafting catalog";
    {
        std::unordered_set<std::string> craftedKeys;
        for (const auto& crafted : data.craftedRecipes) {
            if (crafted.recipeId.empty() || crafted.count == 0 ||
                !craftedKeys.emplace(std::string(crafted.recipeId.value())).second)
                return "duplicate or invalid crafted recipe record";
            if (catalogs.crafting && !catalogs.crafting->find(crafted.recipeId))
                return "crafted recipe record references unknown recipe";
        }
    }
    return {};
}
...
std::vector<std::uint8_t> serializeSave(const SaveData& data){
    ...
    if(!data.craftedRecipes.empty()){
        auto crafted = data.craftedRecipes;
        std::sort(crafted.begin(), crafted.end(), [](const auto& left, const auto& right) {
            return left.recipeId.value() < right.recipeId.value();
        });
        ByteWriter craftedChunk;
        craftedChunk.writeU32(static_cast<std::uint32_t>(crafted.size()));
        for(const auto& value:crafted){
            craftedChunk.writeU32(stringIndex(strings,value.recipeId.value()));
            craftedChunk.writeU32(value.count);
        }
        chunk(chunks, "CRFT", std::move(craftedChunk));
    }
...
SaveResult deserializeSave(std::span<const std::uint8_t> bytes,const SaveValidationCatalogs& catalogs){
    ...
    if(const auto found=chunks.find("CRFT");found!=chunks.end()){ByteReader r(found->second);std::uint32_t count{};if(!r.readU32(count)||count>maps::MapLimits::maximumStrings)return fail("invalid crafted recipe count");for(std::uint32_t i=0;i<count;++i){CraftedRecipeRecord record;if(!readId(r,strings,record.recipeId)||!r.readU32(record.count))return fail("invalid crafted recipe record");data.craftedRecipes.push_back(std::move(record));}if(r.remaining())return fail("trailing crafted recipe data");}
