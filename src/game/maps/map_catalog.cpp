...
TransitionResult MapSession::restore(const simulation::MapId& mapId,const save::SessionWorldState& restoredState){auto loaded=maps_.load(mapId,&catalogs_);if(!loaded)return {false,{},loaded.error};if(loaded.data.playerSpawns.empty())return {false,{},"saved map has no player spawn"};auto built=builder_.build(loaded.data,handles_,loaded.data.playerSpawns.front().id);if(!built)return {false,{},built.error};std::string applyError;if(!save::applyWorldState(restoredState,*built.world,handles_,*catalogs_.items,catalogs_.pickups?*catalogs_.pickups:noPickups(),applyError)){destroyRuntimeHandles(*built.world);return {false,{},applyError};}suppressCompletedEncounterParticipants(loaded.data,*built.world,handles_,restoredState);const PlayerSpawn spawn=built.world->spawn();if(world_)destroyRuntimeHandles(*world_);state_=restoredState;data_=std::move(loaded.data);world_=std::move(built.world);pending_.reset();transitionLatch_=1;return {true,spawn,{}};}
