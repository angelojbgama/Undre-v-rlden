...
void GamePresentation::renderCraftingOverlay(render::Renderer2D& renderer,
    ...
    renderer.fillRect({6, 52, 260, 145}, {8, 10, 16, 245});
    render::drawText(renderer, frame.font, "INVENTORY", 10, 55);
    // Tabs: the inventory grid is the base tab; crafting and its book live
    // here, always available, selected with SECONDARY.
    const char* tabs = "ITEMS  CRAFTING  BOOK";
    render::drawText(renderer, frame.font, tabs, 150, 55);
    const auto highlightTab = [&](int x, int width) {
        renderer.fillRect({x, 53, width, 11}, {96, 62, 54, 160});
    };
    if (view.craftingTab == gameplay::CraftingTab::craft) { highlightTab(158, 52); }
    else { highlightTab(214, 28); }
    if (view.craftingTab == gameplay::CraftingTab::craft) {
        std::size_t listed = 0;
        for (std::size_t index = 0; index < view.craftingRecipes.size(); ++index) {
            ...
            render::drawText(renderer, frame.font,
                             name + (recipe.craftable ? " OK" : (recipe.known ? " --" : " ??")), 11, y);
            ++listed;
        }
        if (view.craftingSelection < view.craftingRecipes.size()) {
            ...
            render::drawText(renderer, frame.font, "QTY " + std::to_string(view.craftingQuantity) +
                             " / MAX " + std::to_string(std::max(recipe.maxCraftable, 1U)), 150, 144);
        }
        if (view.craftingFeedback) {
            ...
            case gameplay::CraftingStatus::recipeLocked: message = "RECIPE LOCKED"; break;
            }
            render::drawText(renderer, frame.font, message, 8, 177);
        }
        render::drawText(renderer, frame.font, "Z/E CRAFT  LEFT/RIGHT QTY  X BOOK  I CLOSE", 8, 188);
        return;
    }
    // Recipe book: every authored recipe is listed, but quest-locked ones show
    // only a silhouette and a hint that a quest will reveal them.
    std::size_t listed = 0;
    for (std::size_t index = 0; index < view.craftingRecipes.size(); ++index) {
        const auto& recipe = view.craftingRecipes[index];
        if (listed == 7) break;
        const int y = 39 + static_cast<int>(listed) * 14;
        const bool selected = index == view.craftingSelection;
        if (selected) { renderer.fillRect({8, y - 1, 250, 13}, {96, 62, 54, 255}); }
        auto name = std::string(recipe.recipeId.value());
        if (name.size() > 18) { name = name.substr(0, 18); }
        const char* marker = recipe.known ? (recipe.craftedCount > 0 ? "*" : "-") : "?";
        render::drawText(renderer, frame.font, marker + std::string(" ") + name, 11, y);
        const int iconX = 220;
        const auto drawVisual = [&](const simulation::DefinitionId& visualId, bool silhouette) {
            if (const auto* sprite = frame.staticSprites.find(visualId)) {
                if (silhouette) {
                    render::drawSpriteSilhouette(renderer, *sprite->sheet, sprite->frame,
                                                 {iconX + 8 + sprite->frame.anchor.x, y + 10 + sprite->frame.anchor.y},
                                                 {70, 70, 86, 255});
                } else {
                    render::drawSprite(renderer, *sprite->sheet, sprite->frame,
                                       {iconX + 8 + sprite->frame.anchor.x, y + 10 + sprite->frame.anchor.y});
        ...
        if (!recipe.outputs.empty()) {
            const auto& output = recipe.outputs.front();
            if (recipe.known) {
                if (output.visualId) { drawVisual(*output.visualId, false); }
                render::drawText(renderer, frame.font, "x" + std::to_string(output.quantity), iconX - 14, y + 1);
            } else if (output.visualId) {
                drawVisual(*output.visualId, true);
            }
        }
        if (recipe.craftedCount > 0) {
            render::drawText(renderer, frame.font, "MADE " + std::to_string(recipe.craftedCount), 160, y);
        } else if (!recipe.known) {
            render::drawText(renderer, frame.font, "QUEST", 160, y);
        }
        ++listed;
    }
    render::drawText(renderer, frame.font, "- KNOWN  * MADE  ? UNDISCOVERED  X CRAFT  I CLOSE", 8, 188);
}
