...
bool playerDef(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayer&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","visualSetId","progressionId","movementCollision"},p,c);AuthoredPlayer d{};bool ok=idField(v,*o,"id",p,c,d.id);ok=idField(v,*o,"visualSetId",p,c,d.visualSetId)&&ok;ok=idField(v,*o,"progressionId",p,c,d.progressionId)&&ok;if(const auto*x=findField(*o,"movementCollision");x&&!isNull(x)){AuthoredPlayerMovementCollision movement{};if(playerMovementCollision(*x,pathOf(p,"movementCollision"),c,movement))d.movementCollision=std::move(movement);else ok=false;}if(ok)out=std::move(d);return ok;}
