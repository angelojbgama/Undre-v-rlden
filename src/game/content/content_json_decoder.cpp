...
bool authoringCategory(const JsonValue& v, std::string_view p, Context& c, AuthoringCategory& out) {
    ...
    else if (s == "npc") out = AuthoringCategory::npc; else if (s == "player") out = AuthoringCategory::player; else if (s == "item") out = AuthoringCategory::item; else if (s == "rewardProfile") out = AuthoringCategory::rewardProfile;
    else if (s == "rewardGrant") out = AuthoringCategory::rewardGrant; else if (s == "shop") out = AuthoringCategory::shop;
    else if (s == "craftingRecipe") out = AuthoringCategory::craftingRecipe;
...
bool craftingIngredient(const JsonValue&v,std::string_view p,Context&c,AuthoredCraftingIngredient&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"itemId","quantity"},p,c);AuthoredCraftingIngredient d{};bool ok=idField(v,*o,"itemId",p,c,d.itemId);ok=unsignedField(v,*o,"quantity",p,c,d.quantity)&&ok;if(ok)out=std::move(d);return ok;}
bool craftingRecipe(const JsonValue&v,std::string_view p,Context&c,AuthoredCraftingRecipe&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","inputs","outputs","unlockQuestId"},p,c);AuthoredCraftingRecipe d{};bool ok=idField(v,*o,"id",p,c,d.id);auto decodeIngredients=[&](const char*n,std::vector<AuthoredCraftingIngredient>&target){const auto*f=required(v,*o,n,p,c);const JsonArray*a=nullptr;if(!f||!array(*f,pathOf(p,n),c,a)){ok=false;return;}for(size_t i=0;i<a->size();++i){AuthoredCraftingIngredient x{};if(craftingIngredient((*a)[i],pathOf(p,n)+"["+std::to_string(i)+"]",c,x))target.push_back(std::move(x));else ok=false;}};decodeIngredients("inputs",d.inputs);decodeIngredients("outputs",d.outputs);if(const auto*quest=findField(*o,"unlockQuestId");quest&&!std::holds_alternative<std::nullptr_t>(quest->value)){std::string s;if(stringValue(*quest,pathOf(p,"unlockQuestId"),c,s))d.unlockQuestId=simulation::DefinitionId{std::move(s)};else ok=false;}if(ok)out=std::move(d);return ok;}
...
bool renderLayerValue(const JsonValue&v,std::string_view p,Context&c,gameplay::ProjectileRenderLayer&out){std::string s;if(!stringValue(v,p,c,s))return false;if(s=="world")out=gameplay::ProjectileRenderLayer::world;else if(s=="actor")out=gameplay::ProjectileRenderLayer::actor;else{c.error(v,std::string(p),"unknown ProjectileRenderLayer");return false;}return true;}
...
bool projectileDrop(const JsonValue&v,std::string_view p,Context&c,AuthoredProjectileDrop&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"pickupId","chancePercent"},p,c);AuthoredProjectileDrop d{};bool ok=idField(v,*o,"pickupId",p,c,d.pickupId);if(const auto*ch=findField(*o,"chancePercent")){ok=unsignedField(*ch,*o,"chancePercent",p,c,d.chancePercent)&&ok;}if(ok&&!d.pickupId.empty())out=d;return ok&&!d.pickupId.empty();}
bool ammoCost(const JsonValue&v,std::string_view p,Context&c,AuthoredAmmoCost&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"itemId","amount"},p,c);AuthoredAmmoCost d{};bool ok=idField(v,*o,"itemId",p,c,d.itemId);if(const auto*am=findField(*o,"amount")){ok=unsignedField(*am,*o,"amount",p,c,d.amount)&&ok;}if(ok&&!d.itemId.empty())out=d;return ok&&!d.itemId.empty();}
bool projectile(const JsonValue&v,std::string_view p,Context&c,AuthoredProjectile&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","visualId","canonicalFacing","speedPixelsPerTick","lifetimeTicks","hitboxWidth","hitboxHeight","spawnOffsets","renderLayer","renderLayers","animationId","maximumDistancePixels","impactAnimationId","expireAnimationId","impactAnimationFacing","expireAnimationFacing","expireAnimations","impactAnimations","impactAnimationFacings","expireAnimationFacings","flipX","impactFlipX","expireFlipX","expireDrop","impactDrop","explosion"},p,c);AuthoredProjectile d{};bool ok=idField(v,*o,"id",p,c,d.id);ok=idField(v,*o,"visualId",p,c,d.visualId)&&ok;const auto*f=required(v,*o,"canonicalFacing",p,c);const auto*s=required(v,*o,"spawnOffsets",p,c);ok=f&&facing(*f,pathOf(p,"canonicalFacing"),c,d.canonicalFacing)&&ok;ok=s&&offsets(*s,pathOf(p,"spawnOffsets"),c,d.spawnOffsets)&&ok;ok=signedField(v,*o,"speedPixelsPerTick",p,c,d.speedPixelsPerTick)&&ok;ok=unsignedField(v,*o,"lifetimeTicks",p,c,d.lifetimeTicks)&&ok;ok=signedField(v,*o,"hitboxWidth",p,c,d.hitboxWidth)&&ok;ok=signedField(v,*o,"hitboxHeight",p,c,d.hitboxHeight)&&ok;if(const auto*layer=findField(*o,"renderLayer")){ok=renderLayerValue(*layer,pathOf(p,"renderLayer"),c,d.renderLayer)&&ok;}if(findField(*o,"animationId")){ok=idField(v,*o,"animationId",p,c,d.animationId)&&ok;}if(findField(*o,"maximumDistancePixels")){ok=unsignedField(v,*o,"maximumDistancePixels",p,c,d.maximumDistancePixels)&&ok;}if(findField(*o,"impactAnimationId")){ok=idField(v,*o,"impactAnimationId",p,c,d.impactAnimationId)&&ok;}if(const auto*impf=findField(*o,"impactAnimationFacing")){ok=facing(*impf,pathOf(p,"impactAnimationFacing"),c,d.impactAnimationFacing)&&ok;}if(const auto*expf=findField(*o,"expireAnimationFacing")){ok=facing(*expf,pathOf(p,"expireAnimationFacing"),c,d.expireAnimationFacing)&&ok;}if(const auto*ia=findField(*o,"impactAnimations")){const JsonObject*io=nullptr;ok=object(*ia,pathOf(p,"impactAnimations"),c,io)&&ok;if(io){for(const auto& m:*io){std::string animName{};if(!stringValue(m.second,pathOf(p,"impactAnimations")+"."+std::string(m.first),c,animName)){ok=false;continue;}simulation::DefinitionId parsed{animName};const std::string_view n=m.first;if(n=="down")d.impactAnimations.values[0]=parsed;else if(n=="up")d.impactAnimations.values[1]=parsed;else if(n=="left")d.impactAnimations.values[2]=parsed;else if(n=="right")d.impactAnimations.values[3]=parsed;}}}if(const auto*imf=findField(*o,"impactAnimationFacings")){const JsonObject*ifo=nullptr;ok=object(*imf,pathOf(p,"impactAnimationFacings"),c,ifo)&&ok;if(ifo){for(const auto& m:*ifo){gameplay::FacingDirection parsed{};if(!facing(m.second,pathOf(p,"impactAnimationFacings")+"."+std::string(m.first),c,parsed)){ok=false;continue;}const std::string_view n=m.first;if(n=="down")d.impactAnimationFacings.values[0]=parsed;else if(n=="up")d.impactAnimationFacings.values[1]=parsed;else if(n=="left")d.impactAnimationFacings.values[2]=parsed;else if(n=="right")d.impactAnimationFacings.values[3]=parsed;}}}if(const auto*exf=findField(*o,"expireAnimationFacings")){const JsonObject*efo=nullptr;ok=object(*exf,pathOf(p,"expireAnimationFacings"),c,efo)&&ok;if(efo){for(const auto& m:*efo){gameplay::FacingDirection parsed{};if(!facing(m.second,pathOf(p,"expireAnimationFacings")+"."+std::string(m.first),c,parsed)){ok=false;continue;}const std::string_view n=m.first;if(n=="down")d.expireAnimationFacings.values[0]=parsed;else if(n=="up")d.expireAnimationFacings.values[1]=parsed;else if(n=="left")d.expireAnimationFacings.values[2]=parsed;else if(n=="right")d.expireAnimationFacings.values[3]=parsed;}}}auto decodeFlip=[&](const char*fieldName,std::array<bool,4>&target)->bool{const auto*fx=findField(*o,fieldName);if(!fx)return true;const JsonObject*fo=nullptr;if(!object(*fx,pathOf(p,fieldName),c,fo))return false;for(const auto& m:*fo){bool enabled=false;if(!boolValue(m.second,pathOf(p,fieldName)+"."+std::string(m.first),c,enabled))return false;const std::string_view n=m.first;if(n=="down")target[0]=enabled;else if(n=="up")target[1]=enabled;else if(n=="left")target[2]=enabled;else if(n=="right")target[3]=enabled;}return true;};ok=decodeFlip("flipX",d.flipX);ok=decodeFlip("impactFlipX",d.impactFlipX)&&ok;ok=decodeFlip("expireFlipX",d.expireFlipX)&&ok;if(findField(*o,"expireAnimationId")){ok=idField(v,*o,"expireAnimationId",p,c,d.expireAnimationId)&&ok;}if(const auto*expAll=findField(*o,"expireAnimations")){const JsonObject*eo=nullptr;ok=object(*expAll,pathOf(p,"expireAnimations"),c,eo)&&ok;if(eo){for(const auto& m:*eo){std::string animName{};if(!stringValue(m.second,pathOf(p,"expireAnimations")+"."+std::string(m.first),c,animName)){ok=false;continue;}simulation::DefinitionId parsed{animName};const std::string_view n=m.first;if(n=="down")d.expireAnimations.values[0]=parsed;else if(n=="up")d.expireAnimations.values[1]=parsed;else if(n=="left")d.expireAnimations.values[2]=parsed;else if(n=="right")d.expireAnimations.values[3]=parsed;}}}if(const auto*layers=findField(*o,"renderLayers")){const JsonObject*lo=nullptr;ok=object(*layers,pathOf(p,"renderLayers"),c,lo)&&ok;if(lo){for(const auto& m:*lo){gameplay::ProjectileRenderLayer parsed{};if(!renderLayerValue(m.second,pathOf(p,"renderLayers")+"."+std::string(m.first),c,parsed)){ok=false;continue;}const std::string_view n=m.first;if(n=="down")d.renderLayers.values[0]=parsed;else if(n=="up")d.renderLayers.values[1]=parsed;else if(n=="left")d.renderLayers.values[2]=parsed;else if(n=="right")d.renderLayers.values[3]=parsed;}}}if(const auto*ed=findField(*o,"expireDrop");ed&&!isNull(ed)){AuthoredProjectileDrop drop{};if(projectileDrop(*ed,pathOf(p,"expireDrop"),c,drop))d.expireDrop=drop;else ok=false;}if(const auto*id2=findField(*o,"impactDrop");id2&&!isNull(id2)){AuthoredProjectileDrop drop{};if(projectileDrop(*id2,pathOf(p,"impactDrop"),c,drop))d.impactDrop=drop;else ok=false;}if(const auto*ex=findField(*o,"explosion");ex&&!isNull(ex)){const JsonObject*xo=nullptr;if(object(*ex,pathOf(p,"explosion"),c,xo)&&xo){AuthoredProjectileExplosion e{};allowed(*xo,{"damageAmount","knockbackPixels","radiusPixels","presentationEffectId"},pathOf(p,"explosion"),c);bool eok=signedField(*ex,*xo,"damageAmount",pathOf(p,"explosion.damageAmount"),c,e.damageAmount);eok=signedField(*ex,*xo,"knockbackPixels",pathOf(p,"explosion.knockbackPixels"),c,e.knockbackPixels)&&eok;eok=signedField(*ex,*xo,"radiusPixels",pathOf(p,"explosion.radiusPixels"),c,e.radiusPixels)&&eok;if(const auto*pe=findField(*xo,"presentationEffectId")){simulation::DefinitionId parsed{};eok=idField(*pe,*xo,"presentationEffectId",pathOf(p,"explosion.presentationEffectId"),c,parsed)&&eok;if(eok)e.presentationEffectId=parsed;}if(eok)d.explosion=e;else ok=false;}}if(ok)out=std::move(d);return ok;}
bool attackEvent(const JsonValue&v,std::string_view p,Context&c,gameplay::AttackTimelineEvent&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"tick","kind","animationId","offsetX","offsetY"},p,c);gameplay::AttackTimelineEvent d{};bool ok=unsignedField(v,*o,"tick",p,c,d.tick);const auto*k=required(v,*o,"kind",p,c);ok=k&&timelineKind(*k,pathOf(p,"kind"),c,d.kind)&&ok;if(ok&&d.kind==gameplay::AttackTimelineEventKind::playEffect){const auto*an=required(v,*o,"animationId",p,c);ok=an&&idField(v,*o,"animationId",p,c,d.effectAnimationId)&&ok;ok=signedField(v,*o,"offsetX",p,c,d.effectOffset.x)&&ok;ok=signedField(v,*o,"offsetY",p,c,d.effectOffset.y)&&ok;}if(ok)out=d;return ok;}
...
bool worldObject(const JsonValue& v, std::string_view p, Context& c, AuthoredWorldObject& out) {
    const JsonObject* o = nullptr;
    if (!object(v, p, c, o)) return false;
    allowed(*o, {"id", "visualSetId", "collision", "depthAnchor", "occlusion", "interactable", "container", "destructible", "bankAccess", "door", "activation", "hazard"}, p, c);
    ...
    if (const auto* x = findField(*o, "hazard"); x) {
        const auto hazardPath = pathOf(p, "hazard");
        const JsonObject* q = nullptr;
        if (!object(*x, hazardPath, c, q)) ok = false;
        else {
            allowed(*q, {"damageAmount", "knockbackPixels", "periodTicks", "activeTicks",
                         "hitbox", "projectileId", "facing", "presentationEffectId"},
                    hazardPath, c);
            gameplay::ObjectHazardDefinition decoded{};
            bool hok = signedField(*x, *q, "damageAmount", pathOf(hazardPath, "damageAmount"), c, decoded.damageAmount);
            if (const auto* knockback = findField(*q, "knockbackPixels")) {
                hok = signedField(*knockback, *q, "knockbackPixels", pathOf(hazardPath, "knockbackPixels"), c, decoded.knockbackPixels) && hok;
            }
            hok = unsignedField(*x, *q, "periodTicks", pathOf(hazardPath, "periodTicks"), c, decoded.periodTicks) && hok;
            hok = unsignedField(*x, *q, "activeTicks", pathOf(hazardPath, "activeTicks"), c, decoded.activeTicks) && hok;
            const auto* hitbox = required(*x, *q, "hitbox", hazardPath, c);
            world::AabbI parsed{};
            hok = hitbox && aabb(*hitbox, pathOf(hazardPath, "hitbox"), c, parsed) && hok;
            decoded.hitbox = parsed;
            if (const auto* projectile = findField(*q, "projectileId")) {
                hok = idField(*projectile, *q, "projectileId", pathOf(hazardPath, "projectileId"), c, decoded.projectileId) && hok;
            }
            if (const auto* facingField = findField(*q, "facing")) {
                hok = facing(*facingField, pathOf(hazardPath, "facing"), c, decoded.facing) && hok;
            }
            if (const auto* effect = findField(*q, "presentationEffectId")) {
                hok = idField(*effect, *q, "presentationEffectId", pathOf(hazardPath, "presentationEffectId"), c, decoded.presentationEffectId) && hok;
            }
            if (hok) d.hazard = decoded;
            else ok = false;
        }
    }
    if (ok) out = std::move(d);
...
bool directional(const JsonValue&v,std::string_view p,Context&c,presentation::DirectionalAnimationRef&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"default","down","up","left","right","side"},p,c);bool ok=true;for(const auto n:{"default","down","up","left","right","side"}){if(const auto*x=findField(*o,n);x&&!isNull(x)){simulation::DefinitionId idValue{};if(!idField(v,*o,n,p,c,idValue))ok=false;else if(std::string_view(n)=="default")out.defaultAnimation=idValue;else if(std::string_view(n)=="down")out.down=idValue;else if(std::string_view(n)=="up")out.up=idValue;else if(std::string_view(n)=="left")out.left=idValue;else if(std::string_view(n)=="right")out.right=idValue;else out.side=idValue;}}if(out.side){if(!out.left)out.left=out.side;if(!out.right)out.right=out.side;}if(!out.defaultAnimation&&!out.down&&!out.up&&!out.left&&!out.right&&!out.side){c.error(v,std::string(p),"directional binding must contain at least one animation");ok=false;}return ok;}
...
bool animationFrameMask(const JsonValue&v,std::string_view p,Context&c,AuthoredAnimationFrameMask&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"channel","width","height","origin","cells"},p,c);AuthoredAnimationFrameMask d{};bool ok=true;const auto*channel=required(v,*o,"channel",p,c);ok=channel&&stringValue(*channel,pathOf(p,"channel"),c,d.channel)&&ok;ok=unsignedField(v,*o,"width",p,c,d.width)&&ok;ok=unsignedField(v,*o,"height",p,c,d.height)&&ok;const auto*origin=required(v,*o,"origin",p,c);ok=origin&&pointI(*origin,pathOf(p,"origin"),c,d.origin)&&ok;const auto*cells=required(v,*o,"cells",p,c);const JsonArray*a=nullptr;if(!cells||!array(*cells,pathOf(p,"cells"),c,a))ok=false;else{d.cells.reserve(a->size());for(std::size_t i=0;i<a->size();++i){std::uint64_t n{};const auto cp=pathOf(p,"cells")+"["+std::to_string(i)+"]";if(!u64((*a)[i],cp,c,n))ok=false;else if(n>1){c.error((*a)[i],cp,"frame mask cells must be 0 or 1");ok=false;}else d.cells.push_back(static_cast<std::uint8_t>(n));}}if(ok)out=std::move(d);return ok;}
bool animationFrame(const JsonValue&v,std::string_view p,Context&c,AuthoredAnimationFrame&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"source","anchor","drawOffset","durationTicks","markers","flipX","masks"},p,c);AuthoredAnimationFrame d{};bool ok=true;const auto*s=required(v,*o,"source",p,c);const auto*a=required(v,*o,"anchor",p,c);const auto*off=required(v,*o,"drawOffset",p,c);const auto*dur=required(v,*o,"durationTicks",p,c);const auto*m=required(v,*o,"markers",p,c);ok=s&&rect(*s,pathOf(p,"source"),c,d.source)&&ok;ok=a&&pointI(*a,pathOf(p,"anchor"),c,d.anchor)&&ok;if(off){const JsonObject*q=nullptr;const auto op=pathOf(p,"drawOffset");if(!object(*off,op,c,q))ok=false;else{allowed(*q,{"x","y"},op,c);if(findField(*q,"x"))ok=signedField(*off,*q,"x",op,c,d.drawOffset.x)&&ok;if(findField(*q,"y"))ok=signedField(*off,*q,"y",op,c,d.drawOffset.y)&&ok;}}else ok=false;if(const auto*flip=findField(*o,"flipX");flip&&!isNull(flip)){bool value{};if(boolValue(*flip,pathOf(p,"flipX"),c,value))d.flipX=value;else ok=false;}ok=dur&&unsignedField(*dur,*o,"durationTicks",p,c,d.durationTicks)&&ok;ok=m&&stringArray(*m,pathOf(p,"markers"),c,d.markers)&&ok;if(const auto*masks=findField(*o,"masks");masks&&!isNull(masks)){const JsonArray*ma=nullptr;if(!array(*masks,pathOf(p,"masks"),c,ma))ok=false;else for(std::size_t i=0;i<ma->size();++i){AuthoredAnimationFrameMask mask{};if(animationFrameMask((*ma)[i],pathOf(p,"masks")+"["+std::to_string(i)+"]",c,mask))d.masks.push_back(std::move(mask));else ok=false;}}if(ok)out=std::move(d);return ok;}
...
bool playerPixelMask(const JsonValue&v,std::string_view p,Context&c,AuthoredPixelMask&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"width","height","origin","cells"},p,c);AuthoredPixelMask d{};bool ok=unsignedField(v,*o,"width",p,c,d.width);ok=unsignedField(v,*o,"height",p,c,d.height)&&ok;const auto*origin=required(v,*o,"origin",p,c);ok=origin&&pointI(*origin,pathOf(p,"origin"),c,d.origin)&&ok;const auto*cells=required(v,*o,"cells",p,c);const JsonArray*a=nullptr;if(!cells||!array(*cells,pathOf(p,"cells"),c,a))ok=false;else{d.cells.reserve(a->size());for(std::size_t i=0;i<a->size();++i){std::uint64_t n{};const auto cp=pathOf(p,"cells")+"["+std::to_string(i)+"]";if(!u64((*a)[i],cp,c,n))ok=false;else if(n>1){c.error((*a)[i],cp,"player mask cells must be 0 or 1");ok=false;}else d.cells.push_back(static_cast<std::uint8_t>(n));}}if(ok)out=std::move(d);return ok;}
bool mirrorPlayerPixelMask(const AuthoredPixelMask& source,AuthoredPixelMask& out){const auto mirroredOrigin=-static_cast<std::int64_t>(source.origin.x)-static_cast<std::int64_t>(source.width);if(mirroredOrigin<std::numeric_limits<int>::min()||mirroredOrigin>std::numeric_limits<int>::max())return false;out=source;out.origin.x=static_cast<int>(mirroredOrigin);out.cells.assign(source.cells.size(),0);for(std::uint32_t y=0;y<source.height;++y)for(std::uint32_t x=0;x<source.width;++x){const auto from=static_cast<std::size_t>(y)*source.width+x;const auto to=static_cast<std::size_t>(y)*source.width+(source.width-1U-x);out.cells[to]=source.cells[from];}return true;}
bool playerMovementCollision(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayerMovementCollision&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"down","up","left","right","side"},p,c);AuthoredPlayerMovementCollision d{};bool ok=true;const auto*down=required(v,*o,"down",p,c);const auto*up=required(v,*o,"up",p,c);ok=down&&playerPixelMask(*down,pathOf(p,"down"),c,d.down)&&ok;ok=up&&playerPixelMask(*up,pathOf(p,"up"),c,d.up)&&ok;const auto*left=findField(*o,"left");const auto*right=findField(*o,"right");const auto*side=findField(*o,"side");if(left&&!isNull(left))ok=playerPixelMask(*left,pathOf(p,"left"),c,d.left)&&ok;if(right&&!isNull(right))ok=playerPixelMask(*right,pathOf(p,"right"),c,d.right)&&ok;if(side&&!isNull(side)){AuthoredPixelMask legacy{};if(playerPixelMask(*side,pathOf(p,"side"),c,legacy)){if(!left||isNull(left))d.left=legacy;if(!right||isNull(right)){AuthoredPixelMask mirrored{};if(mirrorPlayerPixelMask(legacy,mirrored))d.right=std::move(mirrored);else{c.error(*side,pathOf(p,"side"),"legacy side collision cannot be mirrored safely");ok=false;}}}else ok=false;}if((!left||isNull(left))&&(!side||isNull(side))){c.error(v,pathOf(p,"left"),"missing required field");ok=false;}if((!right||isNull(right))&&(!side||isNull(side))){c.error(v,pathOf(p,"right"),"missing required field");ok=false;}if(ok)out=std::move(d);return ok;}
bool playerDef(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayer&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","visualSetId","progressionId","movementCollision","hurtbox"},p,c);AuthoredPlayer d{};bool ok=idField(v,*o,"id",p,c,d.id);ok=idField(v,*o,"visualSetId",p,c,d.visualSetId)&&ok;ok=idField(v,*o,"progressionId",p,c,d.progressionId)&&ok;if(const auto*x=findField(*o,"movementCollision");x&&!isNull(x)){AuthoredPlayerMovementCollision movement{};if(playerMovementCollision(*x,pathOf(p,"movementCollision"),c,movement))d.movementCollision=std::move(movement);else ok=false;}if(const auto*x=findField(*o,"hurtbox");x&&!isNull(x)){AuthoredPixelMask hurtbox{};if(playerPixelMask(*x,pathOf(p,"hurtbox"),c,hurtbox))d.hurtbox=std::move(hurtbox);else ok=false;}if(ok)out=std::move(d);return ok;}
bool playerVisualAction(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayerActionVisual&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"actionId","clips"},p,c);AuthoredPlayerActionVisual d{};const auto*a=required(v,*o,"actionId",p,c);const auto*clips=required(v,*o,"clips",p,c);bool ok=a&&stringValue(*a,pathOf(p,"actionId"),c,d.actionId);ok=clips&&directional(*clips,pathOf(p,"clips"),c,d.clips)&&ok;if(ok)out=std::move(d);return ok;}
bool playerVisual(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayerVisual&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","idle","walk","hurt","actions"},p,c);AuthoredPlayerVisual d{};bool ok=idField(v,*o,"id",p,c,d.id);const auto*idle=required(v,*o,"idle",p,c);const auto*walk=required(v,*o,"walk",p,c);ok=idle&&directional(*idle,pathOf(p,"idle"),c,d.idle)&&ok;ok=walk&&directional(*walk,pathOf(p,"walk"),c,d.walk)&&ok;if(const auto*h=findField(*o,"hurt");h&&!isNull(h)){presentation::DirectionalAnimationRef refs{};if(directional(*h,pathOf(p,"hurt"),c,refs))d.hurt=std::move(refs);else ok=false;}const auto*actions=required(v,*o,"actions",p,c);const JsonArray*aa=nullptr;if(!actions||!array(*actions,pathOf(p,"actions"),c,aa))ok=false;else for(std::size_t i=0;i<aa->size();++i){AuthoredPlayerActionVisual action{};if(playerVisualAction((*aa)[i],pathOf(p,"actions")+"["+std::to_string(i)+"]",c,action))d.actions.push_back(std::move(action));else ok=false;}if(ok)out=std::move(d);return ok;}
...
bool objectVisual(const JsonValue&v,std::string_view p,Context&c,AuthoredWorldObjectVisual&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","idleAnimationId","openedAnimationId","destroyingAnimationId","activationInactiveAnimationId","activationActiveAnimationId","doorLockedAnimationId","doorClosedAnimationId","doorOpenAnimationId","destroyedAnimationId","damagedAnimationId"},p,c);AuthoredWorldObjectVisual d{};bool ok=idField(v,*o,"id",p,c,d.id);ok=idField(v,*o,"idleAnimationId",p,c,d.idleAnimationId)&&ok;for(auto n:{"openedAnimationId","destroyingAnimationId","activationInactiveAnimationId","activationActiveAnimationId","doorLockedAnimationId","doorClosedAnimationId","doorOpenAnimationId","destroyedAnimationId","damagedAnimationId"}){if(const auto*x=findField(*o,n)){std::string s;if(stringValue(*x,pathOf(p,n),c,s)){simulation::DefinitionId value{std::move(s)};if(std::string_view(n)=="openedAnimationId")d.openedAnimationId=value;else if(std::string_view(n)=="destroyingAnimationId")d.destroyingAnimationId=value;else if(std::string_view(n)=="activationInactiveAnimationId")d.activationInactiveAnimationId=value;else if(std::string_view(n)=="activationActiveAnimationId")d.activationActiveAnimationId=value;else if(std::string_view(n)=="doorLockedAnimationId")d.doorLockedAnimationId=value;else if(std::string_view(n)=="doorClosedAnimationId")d.doorClosedAnimationId=value;else if(std::string_view(n)=="doorOpenAnimationId")d.doorOpenAnimationId=value;else if(std::string_view(n)=="damagedAnimationId")d.damagedAnimationId=value;else d.destroyedAnimationId=value;}else ok=false;}}if(ok)out=std::move(d);return ok;}
bool uiAnchor(const JsonValue&v,std::string_view p,Context&c,ui::Anchor&out){std::string s;if(!stringValue(v,p,c,s))return false;if(s=="topLeft")out=ui::Anchor::topLeft;else if(s=="topCenter")out=ui::Anchor::topCenter;else if(s=="topRight")out=ui::Anchor::topRight;else if(s=="centerLeft")out=ui::Anchor::centerLeft;else if(s=="center")out=ui::Anchor::center;else if(s=="centerRight")out=ui::Anchor::centerRight;else if(s=="bottomLeft")out=ui::Anchor::bottomLeft;else if(s=="bottomCenter")out=ui::Anchor::bottomCenter;else if(s=="bottomRight")out=ui::Anchor::bottomRight;else{c.error(v,std::string(p),"unknown ui Anchor");return false;}return true;}
bool uiComponent(const JsonValue&v,std::string_view p,Context&c,ui::ComponentKind&out){std::string s;if(!stringValue(v,p,c,s))return false;if(s=="group")out=ui::ComponentKind::group;else if(s=="panel")out=ui::ComponentKind::panel;else if(s=="image")out=ui::ComponentKind::image;else if(s=="animatedImage")out=ui::ComponentKind::animatedImage;else if(s=="text")out=ui::ComponentKind::text;else if(s=="meter")out=ui::ComponentKind::meter;else if(s=="slot")out=ui::ComponentKind::slot;else if(s=="repeater")out=ui::ComponentKind::repeater;else{c.error(v,std::string(p),"unknown ui ComponentKind");return false;}return true;}
bool uiMeterMode(const JsonValue&v,std::string_view p,Context&c,ui::MeterMode&out){std::string s;if(!stringValue(v,p,c,s))return false;if(s=="segmented")out=ui::MeterMode::segmented;else if(s=="fillHorizontal")out=ui::MeterMode::fillHorizontal;else if(s=="fillVertical")out=ui::MeterMode::fillVertical;else{c.error(v,std::string(p),"unknown ui MeterMode");return false;}return true;}
bool uiConditionOperator(const JsonValue&v,std::string_view p,Context&c,ui::ConditionOperator&out){std::string s;if(!stringValue(v,p,c,s))return false;if(s=="equal")out=ui::ConditionOperator::equal;else if(s=="lessOrEqual")out=ui::ConditionOperator::lessOrEqual;else if(s=="greaterOrEqual")out=ui::ConditionOperator::greaterOrEqual;else{c.error(v,std::string(p),"unknown ui ConditionOperator");return false;}return true;}
bool uiBindingPath(const JsonValue&v,std::string_view p,Context&c,ui::BindingPath&out){std::string s;if(!stringValue(v,p,c,s))return false;if(const auto found=ui::findBindingPath(s)){out=*found;return true;}c.error(v,std::string(p),"unknown ui BindingPath");return false;}
bool uiActionId(const JsonValue&v,std::string_view p,Context&c,ui::ActionId&out){std::string s;if(!stringValue(v,p,c,s))return false;if(const auto found=ui::findAction(s)){out=*found;return true;}c.error(v,std::string(p),"unknown ui ActionId");return false;}
bool uiLayout(const JsonValue&v,std::string_view p,Context&c,ui::LayoutDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"anchor","offsetX","offsetY","width","height","z","visible"},p,c);ui::LayoutDefinition d{};bool ok=true;const auto*a=required(v,*o,"anchor",p,c);ok=a&&uiAnchor(*a,pathOf(p,"anchor"),c,d.anchor)&&ok;ok=signedField(v,*o,"offsetX",p,c,d.offsetX)&&ok;ok=signedField(v,*o,"offsetY",p,c,d.offsetY)&&ok;if(findField(*o,"width")){int width{};ok=signedField(v,*o,"width",p,c,width)&&ok;if(ok)d.width=width;}if(findField(*o,"height")){int height{};ok=signedField(v,*o,"height",p,c,height)&&ok;if(ok)d.height=height;}if(findField(*o,"z")){ok=signedField(v,*o,"z",p,c,d.z)&&ok;}if(const auto*x=findField(*o,"visible");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ok=boolValue(*x,pathOf(p,"visible"),c,d.visible)&&ok;}if(ok)out=d;return ok;}
bool uiMeter(const JsonValue&v,std::string_view p,Context&c,ui::MeterDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"mode","segmentValue","sprites","spacing","emptyRect"},p,c);ui::MeterDefinition d{};bool ok=true;const auto*m=required(v,*o,"mode",p,c);ok=m&&uiMeterMode(*m,pathOf(p,"mode"),c,d.mode)&&ok;ok=unsignedField(v,*o,"segmentValue",p,c,d.segmentValue)&&ok;if(findField(*o,"spacing")){ok=signedField(v,*o,"spacing",p,c,d.spacing)&&ok;}const auto*sp=required(v,*o,"sprites",p,c);const JsonObject*so=nullptr;if(!sp||!object(*sp,pathOf(p,"sprites"),c,so))ok=false;else{const auto spp=pathOf(p,"sprites");allowed(*so,{"fill","full","half","empty"},spp,c);for(auto n:{"fill","full","half","empty"}){const auto*x=findField(*so,n);if(!x||std::holds_alternative<std::nullptr_t>(x->value))continue;std::string s;if(!stringValue(*x,spp+"."+n,c,s))ok=false;else{simulation::DefinitionId parsed{std::move(s)};if(std::string_view(n)=="fill")d.sprites.fill=parsed;else if(std::string_view(n)=="full")d.sprites.full=parsed;else if(std::string_view(n)=="half")d.sprites.half=parsed;else d.sprites.empty=parsed;}}}if(const auto*er=findField(*o,"emptyRect");er&&!std::holds_alternative<std::nullptr_t>(er->value)){const JsonObject*eo=nullptr;const auto erp=pathOf(p,"emptyRect");if(!object(*er,erp,c,eo))ok=false;else{allowed(*eo,{"color","offsetX","offsetY","width","height"},erp,c);ui::MeterEmptyRect rect{};const auto*colorValue=required(*er,*eo,"color",erp,c);ok=colorValue&&color(*colorValue,pathOf(erp,"color"),c,rect.color)&&ok;ok=signedField(*er,*eo,"offsetX",erp,c,rect.offsetX)&&ok;ok=signedField(*er,*eo,"offsetY",erp,c,rect.offsetY)&&ok;ok=signedField(*er,*eo,"width",erp,c,rect.width)&&ok;ok=signedField(*er,*eo,"height",erp,c,rect.height)&&ok;if(ok)d.emptyRect=rect;}}if(ok)out=d;return ok;}
bool uiStateCondition(const JsonValue&v,std::string_view p,Context&c,ui::StateCondition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"source","operator","value"},p,c);ui::StateCondition d{};bool ok=true;const auto*s=required(v,*o,"source",p,c);ok=s&&uiBindingPath(*s,pathOf(p,"source"),c,d.source)&&ok;const auto*op=required(v,*o,"operator",p,c);ok=op&&uiConditionOperator(*op,pathOf(p,"operator"),c,d.op)&&ok;ok=signedField(v,*o,"value",p,c,d.value)&&ok;if(ok)out=d;return ok;}
bool uiVisualDelta(const JsonValue&v,std::string_view p,Context&c,ui::VisualDelta&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"tint","alpha","sprite","visible","background"},p,c);ui::VisualDelta d{};bool ok=true;if(const auto*x=findField(*o,"background");x&&!std::holds_alternative<std::nullptr_t>(x->value)){core::ColorRGBA8 bg{};ok=color(*x,pathOf(p,"background"),c,bg)&&ok;if(ok)d.background=bg;}if(const auto*x=findField(*o,"tint");x&&!std::holds_alternative<std::nullptr_t>(x->value)){core::ColorRGBA8 tint{};ok=color(*x,pathOf(p,"tint"),c,tint)&&ok;if(ok)d.tint=tint;}if(const auto*x=findField(*o,"alpha");x&&!std::holds_alternative<std::nullptr_t>(x->value)){std::uint8_t alpha{};ok=unsignedField(*x,*o,"alpha",p,c,alpha)&&ok;if(ok)d.alpha=alpha;}if(const auto*x=findField(*o,"sprite");x&&!std::holds_alternative<std::nullptr_t>(x->value)){std::string s;ok=stringValue(*x,pathOf(p,"sprite"),c,s)&&ok;if(ok)d.sprite=simulation::DefinitionId{std::move(s)};}if(const auto*x=findField(*o,"visible");x&&!std::holds_alternative<std::nullptr_t>(x->value)){bool value{};ok=boolValue(*x,pathOf(p,"visible"),c,value)&&ok;if(ok)d.visible=value;}if(ok)out=d;return ok;}
bool uiState(const JsonValue&v,std::string_view p,Context&c,ui::StateDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","condition","visual"},p,c);ui::StateDefinition d{};bool ok=true;const auto*id=required(v,*o,"id",p,c);ok=id&&stringValue(*id,pathOf(p,"id"),c,d.id)&&ok;if(const auto*x=findField(*o,"condition");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ui::StateCondition cond{};if(uiStateCondition(*x,pathOf(p,"condition"),c,cond))d.condition=cond;else ok=false;}const auto*vis=required(v,*o,"visual",p,c);ui::VisualDelta delta{};ok=vis&&uiVisualDelta(*vis,pathOf(p,"visual"),c,delta)&&ok;if(ok)d.visual=delta;if(ok)out=std::move(d);return ok;}
bool uiBinding(const JsonValue&v,std::string_view p,Context&c,ui::BindingDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"property","source"},p,c);ui::BindingDefinition d{};bool ok=true;const auto*pr=required(v,*o,"property",p,c);ok=pr&&stringValue(*pr,pathOf(p,"property"),c,d.property)&&ok;const auto*so=required(v,*o,"source",p,c);ok=so&&uiBindingPath(*so,pathOf(p,"source"),c,d.source)&&ok;if(ok)out=std::move(d);return ok;}
bool uiAction(const JsonValue&v,std::string_view p,Context&c,ui::ActionDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"event","action"},p,c);ui::ActionDefinition d{};bool ok=true;const auto*e=required(v,*o,"event",p,c);ok=e&&stringValue(*e,pathOf(p,"event"),c,d.event)&&ok;const auto*a=required(v,*o,"action",p,c);ok=a&&uiActionId(*a,pathOf(p,"action"),c,d.action)&&ok;if(ok)out=std::move(d);return ok;}
bool uiNode(const JsonValue&v,std::string_view p,Context&c,ui::NodeDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","component","layout","sprite","animation","text","meter","bindings","states","actions","children","background","backgroundImage","iconOffset","countOffset","columns","cellWidth","cellHeight","countAlways"},p,c);ui::NodeDefinition d{};bool ok=true;const auto*id=required(v,*o,"id",p,c);ok=id&&stringValue(*id,pathOf(p,"id"),c,d.id)&&ok;const auto*comp=required(v,*o,"component",p,c);ok=comp&&uiComponent(*comp,pathOf(p,"component"),c,d.component)&&ok;const auto*lay=required(v,*o,"layout",p,c);ui::LayoutDefinition layout{};ok=lay&&uiLayout(*lay,pathOf(p,"layout"),c,layout)&&ok;if(ok)d.layout=layout;if(const auto*x=findField(*o,"sprite");x&&!std::holds_alternative<std::nullptr_t>(x->value)){std::string s;ok=stringValue(*x,pathOf(p,"sprite"),c,s)&&ok;if(ok)d.spriteId=simulation::DefinitionId{std::move(s)};}if(const auto*x=findField(*o,"animation");x&&!std::holds_alternative<std::nullptr_t>(x->value)){std::string s;ok=stringValue(*x,pathOf(p,"animation"),c,s)&&ok;if(ok)d.animationId=simulation::DefinitionId{std::move(s)};}if(const auto*x=findField(*o,"text");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ok=stringValue(*x,pathOf(p,"text"),c,d.text)&&ok;}if(const auto*x=findField(*o,"meter");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ui::MeterDefinition meter{};if(uiMeter(*x,pathOf(p,"meter"),c,meter))d.meter=meter;else ok=false;}if(const auto*x=findField(*o,"background");x&&!std::holds_alternative<std::nullptr_t>(x->value)){core::ColorRGBA8 bg{};ok=color(*x,pathOf(p,"background"),c,bg)&&ok;if(ok)d.background=bg;}
if(const auto*x=findField(*o,"backgroundImage");x&&!std::holds_alternative<std::nullptr_t>(x->value)){const JsonObject*q=nullptr;const auto bp=pathOf(p,"backgroundImage");ui::BackgroundImageDefinition frame{};if(!object(*x,bp,c,q))ok=false;else{allowed(*q,{"sprite","border"},bp,c);const auto*sp=required(*x,*q,"sprite",bp,c);const auto*bd=required(*x,*q,"border",bp,c);std::string sprite;if(sp&&stringValue(*sp,pathOf(bp,"sprite"),c,sprite))frame.sprite=simulation::DefinitionId{std::move(sprite)};else ok=false;ok=bd&&signedField(*x,*q,"border",bp,c,frame.border)&&ok;}if(ok)d.backgroundImage=frame;}
if(const auto*x=findField(*o,"iconOffset");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ok=pointI(*x,pathOf(p,"iconOffset"),c,d.iconOffset)&&ok;}
if(const auto*x=findField(*o,"countOffset");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ok=pointI(*x,pathOf(p,"countOffset"),c,d.countOffset)&&ok;}
if(findField(*o,"columns")){ok=signedField(v,*o,"columns",p,c,d.columns)&&ok;}if(findField(*o,"cellWidth")){ok=signedField(v,*o,"cellWidth",p,c,d.cellWidth)&&ok;}if(findField(*o,"cellHeight")){ok=signedField(v,*o,"cellHeight",p,c,d.cellHeight)&&ok;}
if(const auto*x=findField(*o,"countAlways");x&&!std::holds_alternative<std::nullptr_t>(x->value)){ok=boolValue(*x,pathOf(p,"countAlways"),c,d.countAlways)&&ok;}auto decodeArray=[&](const char*n,auto&&parse){if(const auto*f=findField(*o,n)){const JsonArray*a=nullptr;if(!array(*f,pathOf(p,n),c,a))ok=false;else for(std::size_t i=0;i<a->size();++i){if(!parse((*a)[i],pathOf(p,n)+"["+std::to_string(i)+"]"))ok=false;}}};decodeArray("bindings",[&](const JsonValue&x,std::string ip){ui::BindingDefinition b{};if(uiBinding(x,ip,c,b)){d.bindings.push_back(std::move(b));return true;}return false;});decodeArray("states",[&](const JsonValue&x,std::string ip){ui::StateDefinition s{};if(uiState(x,ip,c,s)){d.states.push_back(std::move(s));return true;}return false;});decodeArray("actions",[&](const JsonValue&x,std::string ip){ui::ActionDefinition a{};if(uiAction(x,ip,c,a)){d.actions.push_back(std::move(a));return true;}return false;});decodeArray("children",[&](const JsonValue&x,std::string ip){ui::NodeDefinition child{};if(uiNode(x,ip,c,child)){d.children.push_back(std::move(child));return true;}return false;});if(ok)out=std::move(d);return ok;}
bool uiScreen(const JsonValue&v,std::string_view p,Context&c,ui::ScreenDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"id","kind","root"},p,c);ui::ScreenDefinition d{};bool ok=idField(v,*o,"id",p,c,d.id);const auto*k=required(v,*o,"kind",p,c);std::string kind;if(k&&stringValue(*k,pathOf(p,"kind"),c,kind)){if(kind=="hud")d.kind=ui::ScreenKind::hud;else if(kind=="overlay")d.kind=ui::ScreenKind::overlay;else if(kind=="screen")d.kind=ui::ScreenKind::screen;else{c.error(*k,pathOf(p,"kind"),"unknown ui ScreenKind");ok=false;}}else ok=false;const auto*r=required(v,*o,"root",p,c);ui::NodeDefinition root{};ok=r&&uiNode(*r,pathOf(p,"root"),c,root)&&ok;if(ok)d.root=std::move(root);if(ok)out=std::move(d);return ok;}
...
ContentJsonDecodeResult decodeAuthoredContentJson(std::string_view text){ContentJsonDecodeResult r;const auto p=engine::data::parseJson(text);for(const auto&d:p.diagnostics)r.diagnostics.push_back({d.location.line,d.location.column,{},d.message});if(!p.value||!r.diagnostics.empty())return r;const auto*root=std::get_if<engine::data::JsonObject>(&p.value->value);if(!root){r.diagnostics.push_back({p.value->span.begin.line,p.value->span.begin.column,{},"top-level JSON value must be an object"});return r;}Context c;allowed(*root,{"format","version","tilesets","projectiles","attacks","behaviors","enemies","items","objects","pickups","npcVisuals","npcs","dialogues","quests","playerProgressions","rewardProfiles","rewardGrants","shops","craftingRecipes","authoringDescriptors","tileSemantics","stamps","presentationEffects","visualImages","staticSprites","animations","enemyVisuals","objectVisuals","players","playerVisuals","uiScreens"},"",c);const auto*f=required(*p.value,*root,"format","",c);std::string fs;if(f&&stringValue(*f,"format",c,fs)&&fs!="dungeon-underworld-content")c.error(*f,"format","invalid format identifier");const auto*v=required(*p.value,*root,"version","",c);if(v){uint64_t x{};if(u64(*v,"version",c,x)){c.schemaVersion=x;if(x<1||x>7)c.error(*v,"version","unsupported schema version");if(x<3&&findField(*root,"presentationEffects"))c.error(*v,"version","presentationEffects require schema version 3");if(x<5&&(findField(*root,"visualImages")||findField(*root,"staticSprites")||findField(*root,"animations")||findField(*root,"enemyVisuals")||findField(*root,"objectVisuals")||findField(*root,"players")||findField(*root,"playerVisuals")))c.error(*v,"version","visual content categories require schema version 5");if(x<6&&findField(*root,"craftingRecipes"))c.error(*v,"version","craftingRecipes require schema version 6");if(x<7&&findField(*root,"uiScreens"))c.error(*v,"version","uiScreens require schema version 7");}}AuthoredContentPack out;category(*root,"tilesets",tileset,out.tilesets,c);category(*root,"projectiles",projectile,out.projectiles,c);category(*root,"attacks",attack,out.attacks,c);category(*root,"behaviors",behavior,out.behaviors,c);category(*root,"enemies",enemy,out.enemies,c);category(*root,"items",item,out.items,c);category(*root,"objects",worldObject,out.objects,c);category(*root,"pickups",pickup,out.pickups,c);category(*root,"npcVisuals",npcVisual,out.npcVisuals,c);category(*root,"npcs",npc,out.npcs,c);category(*root,"dialogues",dialogue,out.dialogues,c);category(*root,"quests",quest,out.quests,c);category(*root,"players",playerDef,out.players,c);category(*root,"playerProgressions",progression,out.playerProgressions,c);category(*root,"rewardProfiles",rewardProfile,out.rewardProfiles,c);category(*root,"rewardGrants",rewardGrant,out.rewardGrants,c);category(*root,"shops",shop,out.shops,c);category(*root,"craftingRecipes",craftingRecipe,out.craftingRecipes,c);category(*root,"authoringDescriptors",descriptor,out.authoringDescriptors,c);category(*root,"tileSemantics",tileSemantic,out.tileSemantics,c);category(*root,"stamps",stamp,out.stamps,c);category(*root,"presentationEffects",presentationEffect,out.presentationEffects,c);category(*root,"visualImages",visualImage,out.visualImages,c);category(*root,"staticSprites",staticSprite,out.staticSprites,c);category(*root,"animations",animation,out.animations,c);category(*root,"enemyVisuals",enemyVisual,out.enemyVisuals,c);category(*root,"objectVisuals",objectVisual,out.objectVisuals,c);category(*root,"playerVisuals",playerVisual,out.playerVisuals,c);category(*root,"uiScreens",uiScreen,out.uiScreens,c);r.diagnostics.insert(r.diagnostics.end(),c.diagnostics.begin(),c.diagnostics.end());r.origins=std::move(c.origins);if(r.diagnostics.empty())r.content=std::move(out);else r.origins.clear();return r;}
}
