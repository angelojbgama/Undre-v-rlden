...
bool uiMeter(const JsonValue&v,std::string_view p,Context&c,ui::MeterDefinition&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"mode","segmentValue","sprites"},p,c);ui::MeterDefinition d{};bool ok=true;const auto*m=required(v,*o,"mode",p,c);ok=m&&uiMeterMode(*m,pathOf(p,"mode"),c,d.mode)&&ok;ok=unsignedField(v,*o,"segmentValue",p,c,d.segmentValue)&&ok;const auto*sp=required(v,*o,"sprites",p,c);const JsonObject*so=nullptr;if(!sp||!object(*sp,pathOf(p,"sprites"),c,so))ok=false;else{const auto spp=pathOf(p,"sprites");allowed(*so,{"fill","full","half","empty"},spp,c);for(auto n:{"fill","full","half","empty"}){const auto*x=findField(*so,n);if(!x||std::holds_alternative<std::nullptr_t>(x->value))continue;std::string s;if(!stringValue(*x,spp+"."+n,c,s))ok=false;else{simulation::DefinitionId parsed{std::move(s)};if(std::string_view(n)=="fill")d.sprites.fill=parsed;else if(std::string_view(n)=="full")d.sprites.full=parsed;else if(std::string_view(n)=="half")d.sprites.half=parsed;else d.sprites.empty=parsed;}}}if(ok)out=d;return ok;}
