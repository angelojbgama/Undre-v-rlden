...
bool animationFrame(const JsonValue&v,std::string_view p,Context&c,AuthoredAnimationFrame&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"source","anchor","drawOffset","durationTicks","markers","flipX"},p,c);AuthoredAnimationFrame d{};bool ok=true;const auto*s=required(v,*o,"source",p,c);const auto*a=required(v,*o,"anchor",p,c);const auto*off=required(v,*o,"drawOffset",p,c);const auto*dur=required(v,*o,"durationTicks",p,c);const auto*m=required(v,*o,"markers",p,c);ok=s&&rect(*s,pathOf(p,"source"),c,d.source)&&ok;ok=a&&pointI(*a,pathOf(p,"anchor"),c,d.anchor)&&ok;if(off){const JsonObject*q=nullptr;const auto op=pathOf(p,"drawOffset");if(!object(*off,op,c,q))ok=false;else{allowed(*q,{"x","y"},op,c);if(findField(*q,"x"))ok=signedField(*off,*q,"x",op,c,d.drawOffset.x)&&ok;if(findField(*q,"y"))ok=signedField(*off,*q,"y",op,c,d.drawOffset.y)&&ok;}}else ok=false;if(const auto*flip=findField(*o,"flipX");flip&&!isNull(flip)){bool value{};if(boolValue(*flip,pathOf(p,"flipX"),c,value))d.flipX=value;else ok=false;}ok=dur&&unsignedField(*dur,*o,"durationTicks",p,c,d.durationTicks)&&ok;ok=m&&stringArray(*m,pathOf(p,"markers"),c,d.markers)&&ok;if(ok)out=std::move(d);return ok;}
