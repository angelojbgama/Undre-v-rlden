...
bool playerMovementCollision(const JsonValue&v,std::string_view p,Context&c,AuthoredPlayerMovementCollision&out){const JsonObject*o=nullptr;if(!object(v,p,c,o))return false;allowed(*o,{"down","up","side"},p,c);AuthoredPlayerMovementCollision d{};bool ok=true;const auto*down=required(v,*o,"down",p,c);const auto*up=required(v,*o,"up",p,c);const auto*side=required(v,*o,"side",p,c);ok=down&&playerPixelMask(*down,pathOf(p,"down"),c,d.down)&&ok;ok=up&&playerPixelMask(*up,pathOf(p,"up"),c,d.up)&&ok;ok=side&&playerPixelMask(*side,pathOf(p,"side"),c,d.side)&&ok;if(ok)out=std::move(d);return ok;}
