...
JsonValue projectile(const AuthoredProjectile& v){JsonObject o;p(o,"id",id(v.id));p(o,"visualId",id(v.visualId));p(o,"canonicalFacing",S(facing(v.canonicalFacing)));p(o,"speedPixelsPerTick",I(v.speedPixelsPerTick));p(o,"lifetimeTicks",U(v.lifetimeTicks));p(o,"hitboxWidth",I(v.hitboxWidth));p(o,"hitboxHeight",I(v.hitboxHeight));p(o,"spawnOffsets",offsets(v.spawnOffsets));p(o,"renderLayer",S(renderLayer(v.renderLayer)));if(v.renderLayers.any()){JsonObject r;if(v.renderLayers.values[0])p(r,"down",S(renderLayer(*v.renderLayers.values[0])));if(v.renderLayers.values[1])p(r,"up",S(renderLayer(*v.renderLayers.values[1])));if(v.renderLayers.values[2])p(r,"left",S(renderLayer(*v.renderLayers.values[2])));if(v.renderLayers.values[3])p(r,"right",S(renderLayer(*v.renderLayers.values[3])));p(o,"renderLayers",O(std::move(r)));}return O(std::move(o));}
