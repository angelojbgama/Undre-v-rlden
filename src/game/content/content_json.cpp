...
JsonValue objectDef(const AuthoredWorldObject& v){JsonObject o;p(o,"id",id(v.id));p(o,"visualSetId",id(v.visualSetId));if(v.interactable)p(o,"interactable",box(v.interactable->bounds));if(v.container){JsonObject q;p(q,"capacity",U(v.container->capacity));p(o,"container",O(std::move(q)));}if(v.destructible){JsonObject q;p(q,"maximumHealth",I(v.destructible->maximumHealth));p(q,"hurtbox",box(v.destructible->hurtbox));p(q,"destructionDurationTicks",U(v.destructible->destructionDurationTicks));p(o,"destructible",O(std::move(q)));}if(v.bankAccess)p(o,"bankAccess",O({}));if(v.door){JsonObject q;p(q,"initialState",S(doorState(v.door->initialState)));p(q,"blockingBounds",box(v.door->blockingBounds));p(o,"door",O(std::move(q)));}if(v.activation){JsonObject q;p(q,"mode",S(activationMode(v.activation->mode)));if(v.activation->mode==gameplay::ObjectActivationMode::interactToggle)p(q,"initialActive",B(v.activation->initialActive));if(v.activation->activationBounds)p(q,"bounds",box(*v.activation->activationBounds));p(o,"activation",O(std::move(q)));}return O(std::move(o));}
...
JsonValue animation(const AuthoredAnimation& v){JsonObject o;p(o,"id",id(v.id));p(o,"imageId",id(v.imageId));p(o,"loop",B(v.loop));p(o,"frames",arrayOf(v.frames,[](const auto& f){JsonObject q;p(q,"source",rect(f.source));p(q,"anchor",point(f.anchor));p(q,"drawOffset",point(f.drawOffset));p(q,"durationTicks",U(f.durationTicks));p(q,"markers",strings(f.markers));return O(std::move(q));}));return O(std::move(o));}
